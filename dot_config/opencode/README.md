...
OpenCode is a desktop AI code tool with MCP (Model Context Protocol) server support. This config mirrors the Claude Code MCP servers from `dot_claude/private_settings.json.tmpl`, enabling consistent AI assistance across tools.
...
- **Sync with Claude:** Update `dot_claude/private_settings.json.tmpl` then sync
