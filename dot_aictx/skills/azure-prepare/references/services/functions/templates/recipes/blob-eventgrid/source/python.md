...
def process_blob_upload(
    source_blob_client: blob.BlobClient, processed_container: blob.ContainerClient
) -> None:
    """
    ...
    logging.info(
        f"Blob Trigger (Event Grid) processed blob\n Name: {blob_name} \n Size: {file_size} bytes"
    )
    ...
    try:
        ...
        processed_container.upload_blob(
            processed_blob_name, blob_data.readall(), overwrite=True
        )
        logging.info(
            f"Processing complete for {blob_name}. Copied to {processed_blob_name}."
        )
