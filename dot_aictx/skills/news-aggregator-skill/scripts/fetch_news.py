...
def enrich_items_with_content(items, max_workers=10):
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_item = {
            executor.submit(fetch_url_content, item["url"]): item for item in items
        }
...
def fetch_weibo(limit=5, keyword=None):
    ...
    try:
        ...
        for item in items:
            ...
            full_url = (
                f"https://s.weibo.com/weibo?q={requests.utils.quote(title)}&Refer=top"
            )
...
def fetch_github(limit=5, keyword=None):
    ...
    try:
        response = requests.get(
            "https://github.com/trending", headers=HEADERS, timeout=10
        )
    except:
        return []
...
def fetch_36kr(limit=5, keyword=None):
    try:
        response = requests.get(
            "https://36kr.com/newsflashes", headers=HEADERS, timeout=10
        )
        soup = BeautifulSoup(response.text, "html.parser")
        items = []
...
def fetch_v2ex(limit=5, keyword=None):
    try:
        # Hot topics json
        data = requests.get(
            "https://www.v2ex.com/api/topics/hot.json", headers=HEADERS, timeout=10
        ).json()
        items = []
        for t in data:
...
def fetch_tencent(limit=5, keyword=None):
    try:
        ...
        data = requests.get(
            url, headers={"Referer": "https://news.qq.com/"}, timeout=10
        ).json()
        items = []
...
def fetch_producthunt(limit=5, keyword=None):
    try:
        ...
        response = requests.get(
            "https://www.producthunt.com/feed", headers=HEADERS, timeout=10
        )
        soup = BeautifulSoup(response.text, "html.parser")
...
def fetch_huggingface_papers(limit=5, keyword=None):
    ...
    try:
        ...
        script_path = os.path.join(
            os.path.dirname(__file__), "fetch_hf_papers_playwright.py"
        )
                ...
                items.append(
                    {
                        ...
                        "time": datetime.now().strftime(
                            "%Y-%m-%d"
                        ),  # Daily Papers are today's papers
...
def fetch_latentspace_ainews(limit=5, keyword=None):
    ...
    try:
        response = requests.get(
            "https://www.latent.space/feed", headers=HEADERS, timeout=15
        )
        response.raise_for_status()
        soup = BeautifulSoup(response.text, "html.parser")
...
def fetch_rss_with_playwright(url, source_name, limit=5):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(
                f"Playwright fetch failed for {source_name}: {result.stderr}",
                file=sys.stderr,
            )
            return []
    except Exception as e:
...
def fetch_ai_newsletters(limit=5, keyword=None):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        futures = {
            executor.submit(fetch_rss_feed, url, name, 3): name
...
def main():
    ...
    for name, url in AI_NEWSLETTER_SOURCES:
        ...
        if "Ben's Bites" in name or "The Rundown" in name:
            sources_map[key] = lambda limit=10, k=None, u=url, n=name: filter_items(
                fetch_rss_with_playwright(u, n, limit), k
            )[:limit]
        else:
    ...
    parser.add_argument(
        "--source",
        default="all",
        help='Source(s) to fetch from (comma-separated). Now supports sub-sources like "chinai", "paulgraham"',
    )
    parser.add_argument(
        "--limit", type=int, default=10, help="Limit per source. Default 10"
    )
    ...
    parser.add_argument(
        "--save",
        action="store_true",
        help="Save output to reports directory (JSON + MD)",
    )
    parser.add_argument(
        "--no-save",
        action="store_true",
        dest="no_save",
        help="Skip saving JSON files to disk (only output to stdout)",
    )
    parser.add_argument("--outdir", help="Custom output directory for saved reports")
    parser.add_argument(
        "--list-sources", action="store_true", help="List all available source keys"
    )
    ...
    if args.keyword and len(results) < MIN_ITEMS:
        sys.stderr.write(
            f"Smart Fill triggered: Found {len(results)} items, filling gaps...\n"
        )
    ...
    if not getattr(args, "no_save", False) and (args.save or args.source != "all"):
        if args.outdir:
            ...
            out_dir = os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "reports", today
            )
