...
PROFILES = {
        ...
        "hn_ai": {
            "sources": [
                (
                    fetch_hackernews,
                    20,
                    "AI,LLM,GPT,DeepSeek,Github Copilot,Claude,OpenAI",
                )
            ],
            "enrich": True,
        },
        "github_trending": {"sources": [(fetch_github, 15, None)], "enrich": True},
    },
    # 2. 财经早报 (Finance)
    "finance": {
        "market_overview": {
    ...
    "tech": {
        ...
        "startups": {
            "sources": [
                (fetch_36kr, 20, "融资,首发,独角兽,创投"),
                (fetch_producthunt, 10, None),
            ],
            "enrich": True,
        },
    },
    ...
    "github": {
        "github_trending": {"sources": [(fetch_github, 20, None)], "enrich": True}
    },
    # 6. AI Daily (AI Deep Dive)
    "ai_daily": {
        "newsletter_picks": {
            "sources": [
                (fetch_ai_newsletters, 100, None)
            ],  # Capture all (approx 30-40)
            "enrich": True,
        },
        "huggingface_papers": {
    ...
    "reading_list": {
        ...
        "podcasts": {"sources": [(fetch_podcasts, 50, None)], "enrich": False},
        "hn_deep": {
...
def fetch_section(section_name, config):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in concurrent.futures.as_completed(future_map):
            ...
            try:
                ...
                print(
                    f"[{section_name}] {fname} returned {len(items)} items",
                    file=sys.stderr,
                )
    ...
    if config["enrich"] and results:
        print(
            f"[{section_name}] Enriching content for {len(results)} items...",
            file=sys.stderr,
        )
...
def save_individual_sources(data, base_dir):
    ...
    # Save
    print(
        f"Saving {len(source_map)} individual source files to {base_dir}...",
        file=sys.stderr,
    )
...
def main():
    ...
    parser.add_argument(
        "--profile", default="general", choices=PROFILES.keys(), help="Briefing Profile"
    )
    parser.add_argument(
        "--outdir", help="Optional output directory for individual files"
    )
    parser.add_argument(
        "--no-save",
        action="store_true",
    ...
    if not args.no_save:
        ...
        print(
            f"Saved unified report and {len(sources_saved)} individual source files to {out_dir}",
            file=sys.stderr,
        )
    else:
