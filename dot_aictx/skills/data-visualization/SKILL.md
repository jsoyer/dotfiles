...
ax.hist(
    df["value"], bins=30, color=PALETTE_CATEGORICAL[0], edgecolor="white", alpha=0.8
)
...
ax.axvline(
    mean_val, color="red", linestyle="--", linewidth=1.5, label=f"Mean: {mean_val:,.1f}"
)
ax.axvline(
    median_val,
    color="green",
...
pivot = df.pivot_table(
    index="row_dim", columns="col_dim", values="metric", aggfunc="sum"
)
...
ax.yaxis.set_major_formatter(
    mticker.FuncFormatter(lambda x, p: format_number(x, "currency"))
)
```
