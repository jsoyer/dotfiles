...
def handle_direct_invoke(event, context):
    ...
    return {"statusCode": 200, "result": event}
```
...
class APIGatewayResponse:
    ...
    def text(self, text: str) -> Dict[str, Any]:
        ...
        return {"statusCode": self.status_code, "headers": self.headers, "body": text}
...
def lambda_handler(event, context):
    ...
    try:
        ...
        return error_response(
            LambdaError("Internal server error"), context.aws_request_id
        )
...
def lambda_handler(event, context):
    ...
    return {"statusCode": 200, "body": json.dumps(context_info)}
```
...
def lambda_handler(event, context):
    StructuredLog.info(
        "Processing request", request_id=context.aws_request_id, path=event.get("path")
    )
