...
def main():
    ...
    parser.add_argument(
        "competitor_domain", help="Competitor domain (without https://)"
    )
    parser.add_argument(
        "--location",
        "-loc",
    ...
    if results:
        ...
        print(
            f"keyword_gaps[{len(all_items)}]{{keyword,volume,difficulty,comp_position}}:"
        )
        for item in all_items:
            ...
            if comp_element and isinstance(comp_element, dict):
                comp_pos = comp_element.get(
                    "rank_absolute", comp_element.get("rank_group", "N/A")
                )
            else:
                comp_pos = "N/A"
