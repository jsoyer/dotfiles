...
def read_excel(file_path: str, sheet_name: str = None) -> dict:
    ...
    result = {"file": file_path, "sheets": {}, "sheet_names": wb.sheetnames}
...
def write_excel(
    file_path: str, data: list, sheet_name: str = "Sheet1", headers: list = None
):
    """写入数据到 Excel 文件"""
    wb = Workbook()
    ws = wb.active
...
def convert_excel(file_path: str, to_format: str = "csv", output_path: str = None):
    ...
    if to_format.lower() == "csv":
        ...
        for sheet_name in excel_file.sheet_names:
            ...
            csv_path = (
                Path(output_path).parent / f"{Path(output_path).stem}_{sheet_name}.csv"
            )
...
def main():
    ...
    convert_parser.add_argument(
        "--to", required=True, choices=["csv", "xlsx"], help="目标格式"
    )
