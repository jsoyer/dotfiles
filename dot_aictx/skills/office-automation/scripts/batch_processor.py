...
def batch_fill_templates(
    folder_path: str, template_path: str, data_file: str, output_folder: str
):
    """批量填充 Word 模板"""
...
def batch_convert_folder(
    folder_path: str, from_format: str, to_format: str, output_folder: str
):
    """批量转换文件格式"""
    folder = Path(folder_path)
    output = Path(output_folder)
...
def main():
    ...
    conv_parser.add_argument(
        "--from", dest="from_fmt", required=True, choices=["xlsx", "csv"]
    )
    conv_parser.add_argument(
        "--to", dest="to_fmt", required=True, choices=["xlsx", "csv"]
    )
