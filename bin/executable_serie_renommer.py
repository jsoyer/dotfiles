...
def titres_de_saison(tv_id, saison, cle, cache):
    if saison in cache:
        return cache[saison]
    url = (
        f"https://api.themoviedb.org/3/tv/{tv_id}/season/{saison}"
...
def main():
    ...
    p.add_argument(
        "--serie", required=True, help="nom de serie a inscrire dans les fichiers"
    )
    p.add_argument(
        "--renommer-dossier",
        metavar="NOUVEAU_NOM",
    ...
    for code in sorted(groupes):
        ...
        titre = titres_de_saison(args.tmdb_id, saison, args.cle, cache).get(
            episodes[0], ""
        )
        if not titre:
        ...
        if not mouvements:
            continue
        principal = next(
            (c for c, _ in mouvements if c.suffix.lower() in VIDEO), mouvements[0][0]
        )
        annexes = len(mouvements) - 1
        print(
            f"  {principal.name[:46]:<46} -> {base[:56]}"
    ...
    if sans_titre:
        print(
            f"  {len(sans_titre)} episode(s) sans titre chez TMDb : "
