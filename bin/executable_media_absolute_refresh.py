    ...
    for show in shows:
        ...
        try:
            mapping = abs_shows.refresh_mapping(
                show, rclone=args.rclone, api_key=api_key, language=language
            )
        ...
        print(
            f"{show.name} : {len(mapping.seasons)} saisons, "
            f"dernier absolu {mapping.last_absolute}"
        )
        print(f"  source : {mapping.source}  (decalage {mapping.offset:+d})")
        if slot:
            print(
                f"  prochain episode attendu : absolu {mapping.last_absolute + 1} "
