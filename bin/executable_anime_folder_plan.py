...
def nom_attendu(dossier, config):
    ...
    candidat = ma.search_tmdb_tv(
        recherche, annee, config.tmdb_api_key, config.tmdb_language
    )
    if not candidat:
        return None, "aucune correspondance TMDb"
    details = ma.get_tmdb_tv_details(
        candidat["id"], config.tmdb_api_key, config.tmdb_language
    )
...
def main():
    ...
    parseur.add_argument(
        "--liste", required=True, type=Path, help="un nom de dossier par ligne"
    )
    parseur.add_argument(
        "--racine", required=True, help="chemin de la bibliotheque sur l hote"
    )
    parseur.add_argument(
        "--config",
        type=Path,
    ...
    print(
        f"  {len(dossiers)} dossiers | conformes : {len(conformes)} | "
