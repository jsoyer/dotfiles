...
ABSOLUTE_PATTERNS = [
    ...
    re.compile(
        r"(?:^|[\s._-])(?:E|EP|EPISODE)[\s._-]?(?P<n>\d{1,4})(?:v\d)?(?=[\s._-]|$)",
        re.I,
    ),
    ...
    for entry in raw.get("show", []):
        ...
        shows.append(
            AbsoluteShow(
                ...
                mapping_file=(path.parent / mapping)
                if not Path(mapping).is_absolute()
                else Path(mapping),
                ...
                absolute_offset=(
                    int(entry["absolute_offset"])
                    if entry.get("absolute_offset") is not None
                    else None
                ),
            )
        )
...
def build_mapping_from_tvdb_order(
    tmdb_id, api_key, language="fr-FR", group_name="TVDB Order", offset=0, timeout=60
):
    ...
    match = next(
        (g for g in listing.get("results", []) if g.get("name") == group_name), None
    )
    if match is None:
        available = ", ".join(
            sorted(g.get("name", "?") for g in listing.get("results", []))
        )
        raise ValueError(
            f"Groupe {group_name!r} absent pour TMDb {tmdb_id}. Disponibles: {available}"
        )
    ...
    return Mapping(
        seasons=seasons,
        last_absolute=0,
        source=f"tmdb:{tmdb_id}:{group_name}",
        offset=offset,
    )
...
def calibrate_offset(seasons, season_counts, total_items):
    ...
    position = next(
        (low + held - 1 for season, low, high in seasons if season == last_season), None
    )
    if position is None:
        return 0
...
def load_mapping(path):
    ...
    return Mapping(
        seasons=[tuple(row) for row in data["seasons"]],
...
def title_from_episode_group(detail, season, episode):
    ...
    """
    group = next(
        (item for item in detail.get("groups", []) if item.get("order") == season), None
    )
    if group is None:
        return None
...
def fetch_episode_title(show, season, episode, api_key, language="fr-FR", timeout=60):
    ...
    match = next(
        (
            group
            for group in listing.get("results", [])
            if group.get("name") == show.order_group
        ),
        None,
    )
    if match is None:
...
def remote_exists(destination, rclone="rclone", timeout=120):
    ...
    result = subprocess.run(
        [rclone, "lsf", destination], capture_output=True, text=True, timeout=timeout
    )
    return result.returncode == 0 and bool(result.stdout.strip())


def push_episode(
    local_path, destination, dry_run=False, rclone="rclone", tpslimit=4, timeout=7200
):
