...
class EpisodesMustParse(unittest.TestCase):
    ...
    def test_episodes_parse_correctly(self):
        for name, title, season, episodes in self.CASES:
            with self.subTest(name=name):
                self.assertEqual(
                    ma.parse_episode_filename(name, name), (title, season, episodes)
                )
    ...
    def test_leading_token_is_parsed(self):
        for name, title, season, episodes in self.LEADING_TOKEN:
            with self.subTest(name=name):
                self.assertEqual(
                    ma.parse_episode_filename(name, "peu importe"),
    ...
    def test_leading_token_without_show_falls_back_to_parent(self):
        ...
        parsed = ma.parse_episode_filename(
            "S18E30 - Pikachu en vedette !.mp4", "Pokemon"
        )
    ...
    def test_leading_token_does_not_swallow_the_episode_title(self):
        ...
        title, _, _ = ma.parse_episode_filename(
            "S18E30 - Pikachu en vedette !.mp4", "Pokemon"
        )
    ...
    def test_classic_naming_still_wins(self):
        ...
        self.assertEqual(
            ma.parse_episode_filename("Breaking.Bad.S03E07.1080p.mkv", "x"),
            ("Breaking Bad", 3, [7]),
        )
...
class TmdbCandidateChoice(unittest.TestCase):
    ...
    RESULTATS = [
        {
            "id": 224499,
            "name": "ちみキャラ劇場",
            "original_name": "ちみキャラ劇場",
            "popularity": 5,
        },
        {
            "id": 63510,
            "name": "L'Attaque des Titans - Junior High School",
            "original_name": "Attack on Titan: Junior High",
            "popularity": 9,
        },
        {
            "id": 1429,
    ...
    def test_original_title_also_counts_as_exact(self):
        ...
        self.assertEqual(
            ma._meilleur_candidat_tv("Attack on Titan", resultats)["id"], 2
        )

    def test_missing_popularity_does_not_crash(self):
        resultats = [
            {"id": 1, "name": "Sans note"},
            {"id": 2, "name": "Autre", "popularity": 3},
        ]
...
class TitleCleaning(unittest.TestCase):
    ...
    def test_season_marker_is_removed(self):
        self.assertEqual(
            ma.clean_series_title("Naruto Shippuden Saison 5"), "Naruto Shippuden"
        )
...
class YearHint(unittest.TestCase):
    ...
    CASES = [
        ...
        (
            "Space.Adventure.Cobra.1982.TV.Series.E01.MULTi.1080p.BluRay.x265.10bits-NSP",
            "1982",
        ),
    ]
...
class AbsoluteNumbering(unittest.TestCase):
    ...
    def test_absolute_maps_to_the_right_season(self):
        ...
        for absolute, expected in cases.items():
            with self.subTest(absolute=absolute):
                self.assertEqual(
                    self.plan.absolute_to_season_episode(absolute, self.table), expected
                )
    ...
    def test_unparsable_names_are_reported_not_renamed(self):
        plan, unresolved = self.plan.plan_renames(
            ["One Piece - 0001.mkv", "bande-annonce.mkv", "lisez-moi.txt"],
            self.table,
            "One Piece",
        )
    ...
    def test_destination_follows_the_plex_convention(self):
        plan, _ = self.plan.plan_renames(
            ["One Piece - 0062.mkv"], self.table, "One Piece"
        )
...
class AbsoluteShowRegistry(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.show = media_absolute_shows.AbsoluteShow(
            name="One Piece",
            destination="gdrive:Infuse/One Piece",
            mapping_file=Path("/nonexistent.json"),
    ...
    def test_registered_show_matches_its_aliases(self):
        for name in [
            "[SubsPlease] One Piece - 1173 (1080p) [A1B2C3D4].mkv",
    ...
    def test_season_organised_names_are_refused(self):
        with self.assertRaises(ValueError):
            self.mod.plan_episode(
                "One Piece - 3x05 - Titre.mkv", self.show, self.mapping
            )

    def test_push_is_a_server_side_move_with_rate_limit(self):
        command = self.mod.push_episode(
            "/inbox/ep.mkv", "gdrive:X/ep.mkv", dry_run=True
        )
...
class EpisodeFilenamesCarryTheirTitle(unittest.TestCase):
    ...
    def test_title_is_embedded(self):
        self.assertEqual(
            self.target(episode_title="Le vieux loup de mer"),
    ...
    def test_illegal_characters_in_the_title_are_stripped(self):
        ...
        self.assertEqual(
            self.target(episode_title="Qui vole un <oeuf> : la suite ?"),
            "Pokemon - S08E09 - Qui vole un oeuf la suite.mkv",
        )

    def test_double_episode_keeps_the_range(self):
        self.assertEqual(
            ma.get_episode_target_name(
                Path("x.mkv"), "Pokemon", 2, [5, 6], episode_title="Deux en un"
            ),
    ...
    def test_nfo_follows_the_same_convention(self):
        self.assertEqual(
            self.target(suffix=".nfo", episode_title="Le vieux loup de mer"),
    ...
    def test_artwork_is_left_alone(self):
        ...
        self.assertEqual(
            ma.get_episode_target_name(
                Path("poster.jpg"), "Pokemon", 8, [9], episode_title="Un titre"
            ),
...
class TheBulkRenamerSpeaksTheImportersLanguage(unittest.TestCase):
    ...
    def test_base_name_matches_the_importer(self):
        ...
        self.assertEqual(
            self.outil.base_cible("Serie", 1, [1], "Un titre") + ".mkv", attendu
        )

    def test_double_episode_keeps_both_numbers(self):
        self.assertEqual(
            self.outil.base_cible("Serie", 2, [5, 6], "Double"),
            "Serie - S02E05-E06 - Double",
        )

    def test_series_name_is_sanitised_by_the_shared_rule(self):
        self.assertEqual(
            self.outil.base_cible("Ma : Serie", 1, [1], "Titre"),
    ...
    def test_the_range_is_captured_not_truncated(self):
        ...
        self.assertEqual(
            (found.group("s"), found.group("e"), found.group("e2")), ("02", "05", "06")
        )
...
class AYearFromASeasonFolderMustNotHideTheSeries(unittest.TestCase):
    ...
    def test_a_year_that_finds_nothing_is_retried_without_it(self):
        ...
        self.assertEqual(
            len(appels), 2, "une seconde recherche, sans annee, est attendue"
        )
    ...
    def test_a_year_that_works_is_not_retried(self):
        ...
        self.assertEqual(trouve["id"], 30983)
        self.assertEqual(
            len(appels), 1, "aucune recherche superflue quand la premiere aboutit"
        )
...
class ASeasonSubtitleMustNotHideTheSeries(unittest.TestCase):
    ...
    def test_a_franchise_prefix_is_not_tried(self):
        ...
        trouve, appels = self.rejouer("Star Trek Discovery", lambda _: {"results": []})
        self.assertIsNone(trouve)
    ...
    def test_an_unknown_two_word_title_still_returns_nothing(self):
        trouve, appels = self.rejouer("Zxqtplm Heroes", lambda _: {"results": []})
        self.assertIsNone(trouve)
...
class LongRunningSeriesMustNotBeTruncated(unittest.TestCase):
    ...
    def test_four_digit_episode_is_read_whole(self):
        self.assertEqual(
            self.parse("Détective Conan - S01E1157 - Enquête à Ishikawa.mkv"),
    ...
    def test_the_highest_numbers_do_not_collapse(self):
        for numero in (1190, 1199, 1200, 1209):
            with self.subTest(numero=numero):
                self.assertEqual(
                    self.parse(f"Détective Conan - S01E{numero} - Titre.mkv")[2],
                    [numero],
                )
    ...
    def test_a_leading_token_also_reads_four_digits(self):
        self.assertEqual(
            self.parse("S01E1157 - Enquête à Ishikawa.mkv", "Détective Conan"),
            ("Détective Conan", 1, [1157]),
        )
...
class AYearInTheTitleIsNotAReleaseYear(unittest.TestCase):
    ...
    def test_an_ordinary_release_still_parses(self):
        self.assertEqual(
            ma.parse_filename("Matrix.1999.MULTi.1080p.BluRay.mkv"), ("Matrix", "1999")
        )
...
class ABracketedReleaseGroupMustNotDefeatTheParser(unittest.TestCase):
    ...
    NOM = (
        "[Delivroozzi] Détective Conan - Le Cauchemar Noir de Jais "
...
class UnNumeroNuApresUnTiret(unittest.TestCase):
    ...
    def test_une_marque_de_reedition_ne_fait_pas_echouer_la_lecture(self):
        ...
        for nom, numero in (
            (
                "[Pokémon Fansub] Pokémon Horizons - 139 (VOSTFR-FR 1920x1080 H264 AAC)v2.mp4",
                139,
            ),
    ...
    def test_un_millesime_ne_devient_pas_un_numero_d_episode(self):
        ...
        for nom in ("Blade Runner - 2049 (2017).mkv", "Le Parrain - 2 (1974).mkv"):
...
class UnEnvoiNtfyViseLeBonSujet(unittest.TestCase):
    ...
    def test_desactive_n_emet_rien(self):
        settings = ma.NtfyConfig(
            enabled=False, server="https://n.test", topic="infra-nice"
        )
    ...
    def test_vise_le_serveur_et_le_sujet(self):
        settings = ma.NtfyConfig(
            enabled=True, server="https://n.test/", topic="infra-nice"
        )
    ...
    def test_le_corps_accepte_les_accents_et_les_emoji(self):
        ...
        self.assertEqual(
            requete.data.decode("utf-8"), "⚠️ episode ecarte : deja present"
        )

    def test_le_jeton_devient_un_entete_bearer(self):
        settings = ma.NtfyConfig(
            enabled=True, server="https://n.test", topic="infra-nice", token="tk_essai"
        )
        (requete,) = self._capturer(settings, "coucou")
    ...
    def test_la_priorite_est_transmise(self):
        settings = ma.NtfyConfig(
            enabled=True, server="https://n.test", topic="infra-nice", priority=4
        )
        (requete,) = self._capturer(settings, "coucou")
    ...
    def test_une_ligne_unique_trop_longue_est_coupee(self):
        ...
        self.assertEqual("".join(r.data.decode("utf-8") for r in requetes), "x" * 9000)
...
class UneNotificationSertLesCanauxActives(unittest.TestCase):
    ...
    def _servis(self, config):
        ...
        with (
            mock.patch.object(
                ma, "send_telegram_message", lambda s, m: servis.append("telegram")
            ),
            mock.patch.object(
                ma, "send_ntfy_message", lambda s, m, t=None: servis.append("ntfy")
            ),
        ):
    ...
    def test_les_deux_canaux_actifs_sont_servis(self):
        self.assertEqual(
            sorted(self._servis(self._config(True, True))), ["ntfy", "telegram"]
        )
...
class UneRessemblanceNeContreditPasUneIdentite(unittest.TestCase):
    ...
    def test_des_identifiants_differents_empechent_la_fusion(self):
        with tempfile.TemporaryDirectory() as brut:
            ...
            self._dossier(racine, "Attaque du dragon", "Dragon Ball Z", tmdbid="39104")
...
class UnNomDeVersionSuitLaConventionEmby(unittest.TestCase):
    ...
    def test_la_qualite_suit_un_tiret_entoure_d_espaces(self):
        self.assertEqual(ma.nom_de_version("300 (2006)", "1080p"), "300 (2006) - 1080p")

    def test_le_nom_du_dossier_ouvre_toujours_le_nom_de_fichier(self):
        dossier = "L'Attaque des Titans (2020)"
        self.assertTrue(ma.nom_de_version(dossier, "2160p").startswith(dossier + " - "))
...
class UnDossierDeQualiteSAplatit(unittest.TestCase):
    ...
    def test_deux_qualites_cohabitent_sans_se_marcher_dessus(self):
        with tempfile.TemporaryDirectory() as brut:
            ...
            ma.aplatir_dossier_qualite(d)
            videos = sorted(f.name for f in d.iterdir() if f.suffix.lower() == ".mkv")
            self.assertEqual(
                videos, ["Avatar (2009) - 1080p.mkv", "Avatar (2009) - 2160p.mkv"]
            )
            # Un seul visuel : les deux etaient homonymes, le second n'ecrase
            # pas le premier et ne se transforme pas en doublon numerote.
            self.assertEqual(
                [f.name for f in d.iterdir() if f.suffix.lower() == ".jpg"],
                ["poster.jpg"],
            )
            self.assertFalse((d / "1080p").exists())
    ...
    def test_l_operation_est_idempotente(self):
        with tempfile.TemporaryDirectory() as brut:
            d = self._film(
                Path(brut), "Le Parrain (1972)", {"1080p": ["Le Parrain (1972).mkv"]}
            )
            ma.aplatir_dossier_qualite(d)
...
class AucunePhaseNeCreuseDeSousDossierDeQualite(unittest.TestCase):
    ...
    def test_aucun_chemin_ne_compose_un_sous_dossier_de_qualite(self):
        ...
        for motif in self.MOTIFS:
            ...
            self.assertEqual(
                fautives, [], f"« {motif} » compose encore un sous-dossier"
            )
...
class UneEtiquetteDeVersionSeLitDansLeNom(unittest.TestCase):
    ...
    def test_la_qualite_est_l_etiquette(self):
        self.assertEqual(
            ma.etiquette_de_version(Path(f"{self.DOSSIER} - 1080p.mkv"), self.DOSSIER),
            "1080p",
        )
    ...
    def test_le_suffixe_de_copie_ne_fait_pas_une_version(self):
        ...
        self.assertEqual(
            ma.etiquette_de_version(
                Path(f"{self.DOSSIER} - 1080p (2).mkv"), self.DOSSIER
            ),
            "1080p",
        )
    ...
    def test_une_edition_est_une_etiquette_comme_une_autre(self):
        self.assertEqual(
            ma.etiquette_de_version(
                Path(f"{self.DOSSIER} - directors cut.mkv"), self.DOSSIER
            ),
            "directors cut",
        )
    ...
    def test_deux_versions_distinctes_ne_sont_pas_un_doublon(self):
        with tempfile.TemporaryDirectory() as brut:
            d = self._dossier(
                Path(brut),
                "Avatar (2009)",
                ["Avatar (2009) - 1080p.mkv", "Avatar (2009) - 2160p.mkv"],
            )
    ...
    def test_deux_fichiers_de_meme_etiquette_sont_un_doublon(self):
        with tempfile.TemporaryDirectory() as brut:
            d = self._dossier(
                Path(brut),
                "Avatar (2009)",
                ["Avatar (2009) - 1080p.mkv", "Avatar (2009) - 1080p (2).mkv"],
            )
    ...
    def test_une_seule_video_ne_signale_rien(self):
        with tempfile.TemporaryDirectory() as brut:
            d = self._dossier(
                Path(brut),
                "Seul (2000)",
                ["Seul (2000) - 1080p.mkv", "poster.jpg", "Seul (2000) - 1080p.nfo"],
            )
            self.assertEqual(ma.doublons_internes(d), [])
    ...
    def test_les_annexes_ne_comptent_pas_comme_des_videos(self):
        with tempfile.TemporaryDirectory() as brut:
            d = self._dossier(
                Path(brut),
                "Seul (2000)",
                ["Seul (2000) - 1080p.mkv", "fanart.jpg", "poster.jpg", "logo.png"],
            )
            self.assertEqual(ma.doublons_internes(d), [])
...
class UnVocabulaireDeTitreSeDegageDuBruit(unittest.TestCase):
    ...
    def test_les_accents_et_la_casse_ne_comptent_pas(self):
        self.assertEqual(
            ma.mots_signifiants("L'Infini"), ma.mots_signifiants("l infini")
        )
...
class UnMemeFilmPeutVivreDansDeuxRacines(unittest.TestCase):
    ...
    def test_trois_copies_forment_un_seul_groupe(self):
        troisieme = (
            "/data/animes/Demon Slayer Films (2020)/"
        ...
        self.assertEqual(
            len(
                self._couple(
                    ...
                    "Le Seigneur des Anneaux La Communaute de l Anneau remastered - 1080p.mkv",
                )
            ),
            1,
        )
        ...
        self.assertEqual(
            len(
                self._couple(
                    ...
                    "Blade Runner final cut - 1080p.mkv",
                )
            ),
            1,
        )
        ...
        self.assertEqual(
            len(
                self._couple(
                    "/data/movies/Blade Runner (1982)/Blade Runner (1982) - 1080p.mkv",
                    "/data/animes/Blade Runner (1982)/"
                    "Blade Runner directors cut remastered - 1080p.mkv",
                )
            ),
            1,
        )
...
class UnCaractereInvisibleNeSurvitPasAuNettoyage(unittest.TestCase):
    ...
    def test_la_marque_gauche_droite_disparait(self):
        self.assertEqual(
            ma.sanitize("‎Des Minions et des Monstres"), "Des Minions et des Monstres"
        )
    ...
    def test_les_autres_caracteres_de_format_aussi(self):
        ...
        for parasite in ("​", "‍", "﻿", "‏"):
            with self.subTest(parasite=hex(ord(parasite))):
                self.assertEqual(ma.sanitize(f"Titre{parasite} propre"), "Titre propre")
...
"""
    ...
    def test_space_check_requires_payload_plus_margin(self):
        ...
        self.assertFalse(
            self.mx.has_room(free=payload, uncompressed=payload, margin=margin)
        )
        self.assertTrue(
            self.mx.has_room(
    ...
    def test_hidden_and_partial_archives_are_ignored(self):
        ...
        found = [
            p.name for p in self.mx.gather_archives(self.inbox, stability_seconds=0)
        ]
    ...
    def test_archive_is_kept_when_disk_is_too_small(self):
        ...
        with mock.patch.object(self.mx, "disk_free", return_value=1):
            results = self.mx.extract_pending_archives(
                self.inbox,
                stability_seconds=0,
                margin_bytes=10 * 1024 * 1024 * 1024,
                dry_run=False,
            )
        self.assertFalse(results[0].extracted)
    ...
    def test_multipart_siblings_are_removed_with_the_primary(self):
        ...
        self.assertEqual(
            {p.name for p in members}, {"Show.part1.rar", "Show.part2.rar"}
        )
