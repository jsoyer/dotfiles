...
ALREADY_RE = re.compile(
    r"^.*?[\s._-]+S(?P<season>\d{1,2})E(?P<episode>\d{1,3})(?:[\s._-]|$)", re.I
)
...
RUNNING_RE = re.compile(
    r"^(?P<prefix>.*?)[\s._-]+-[\s._-]+(?P<number>\d{2,4})"
    r"(?P<rest>[\s._-]+-[\s._-]+.*)?$"
)
...
def build_plan(entries, show_name, derive_orders):
    ...
    for season in sorted(by_season):
        ...
        for entry, stem, suffix, match in tagged:
            ...
            if declared != season:
                unresolved.append(
                    (
                        entry,
                        f"le fichier annonce la saison {declared}, le dossier {season}",
                    )
                )
                continue
        ...
        if season not in derive_orders:
            for entry, _, _, _ in running:
                unresolved.append(
                    (
                        entry,
                        f"numero brut ; utiliser --derive-order {season} apres verification",
                    )
                )
            continue
        if tagged:
        ...
        if len(numbers) != len(set(numbers)) or expected != len(numbers):
            for entry, _, _, _ in running:
                unresolved.append(
                    (
                        entry,
                        f"saison {season} : numeros non contigus ({numbers[0]}-{numbers[-1]} "
        ...
        offset = numbers[0]
        for entry, _, suffix, match in sorted(
            running, key=lambda r: int(r[3].group("number"))
        ):
            episode = int(match.group("number")) - offset + 1
            rest = (match.group("rest") or "").strip(" -._")
            plan.append(
                _make_rename(
                    entry, season, episode, show_name, rest, suffix, derived=True
                )
            )
...
def main():
    ...
    parser.add_argument(
        "--show", required=True, help="series name used in the new filenames"
    )
    parser.add_argument("--list", required=True, type=Path, help="rclone lsf -R output")
    parser.add_argument(
        "--remote", required=True, help='rclone path, e.g. "gdrive:Infuse/One Piece"'
    )
    parser.add_argument("--dest", help="destination rclone path (defaults to --remote)")
    parser.add_argument(
        "--out", required=True, type=Path, help="output prefix (.txt and .sh)"
    )
    parser.add_argument(
        "--derive-order",
        type=int,
    ...
    if collisions:
        for destination, sources in collisions.items():
            unresolved.extend(
                (source, f"collision sur {destination}") for source in sources
            )
