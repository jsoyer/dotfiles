...
def main():
    ...
    parser.add_argument(
        "--show", required=True, help="series title as searched on TMDb"
    )
    ...
    parser.add_argument(
        "--remote", required=True, help="rclone source path, e.g. gdrive:Anime/OnePiece"
    )
    ...
    candidate = ma.search_tmdb_tv(
        args.show, args.year, config.tmdb_api_key, config.tmdb_language
    )
    if not candidate:
        sys.exit(f"Aucune serie TMDb pour {args.show!r}")
    details = ma.get_tmdb_tv_details(
    ...
    lines += [
        f"  S{s:02d} : absolus {lo}-{hi} ({n} episodes)" for s, lo, hi, n in table
    ]
    lines += ["", "## Renommages"]
    lines += [
